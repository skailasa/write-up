...
\begin{flalign}
    |\langle f_n, \phi \rangle| = |\int f_n \phi dx | \leq \|f_n\|_{L_2} \|\phi\|_{L_2} \rightarrow 0
\end{flalign}
...
\begin{flalign}
    ...
    &\leq C \sup_{y \in \reals^N} \left(|\phi(y)|^2\tilde{C}(1+|y|^{2N})\right) \\
    &\leq C \tilde{C} \left(\|\phi\|^2_{0,0} + \|\phi\|^2_{N, 0}\right)
\end{flalign}
...
\begin{flalign}
    \|\hat{u}_n - \hat{u}_m \| = (2\pi)^{N/2}\|u_n-u_m\|_{L_2} \rightarrow 0
\end{flalign}
...
\begin{flalign}
    \| \hat{u} \|_{L_2}^2 = (2\pi)^N\|u\|_{L_2}^2
\end{flalign}
...
\begin{flalign}
    \|\hat{u}\|_{\lone} &= \int_{\reals^N} |\hat{u}(\zeta)| d\zeta = \int|\hat{u}(\zeta)| (1+|\zeta|^2)^{s/2}(1+|\zeta|^2)^{-s/2} d\zeta \\
    &\overset{\text{Cauchy Schwarz}}{\leq} \left(\int_{\reals^N} |\hat{u}|^2(1+|\zeta|^2)^s d\zeta \right)^{1/2} \left( \int_{\reals^N} (1+|\zeta|^2)^{-s}\right)^{1/2} \\
    &\leq C \|u\|_{H^s}
\end{flalign}
...
\begin{flalign}
    \hat{f} = \widehat{-\Delta u + 5u}
\end{flalign}

By operation rules,

\begin{flalign}
    = |\zeta|^2 \hat{u} + 5\hat{u} = (|\zeta|^2+5) \frac{\hat{f}(\zeta)}{|\zeta|^2+5} = \hat{f}(\zeta)
\end{flalign}
...
\begin{flalign}
    u \in H^2 \Leftrightarrow (1+|\zeta|^2)\hat{u} \in L_2 \Leftrightarrow \frac{\hat{f}(\zeta)}{1+|\zeta|^1}(1+|\zeta|^2) \in L_2 \Leftrightarrow \hat{f}(\zeta) \in L_2 \Leftrightarrow f \in L_2
\end{flalign}
...
\begin{flalign}
    u = \mathcal{F}^{-1}\left[ \frac{\hat{f}(\zeta)}{|\zeta|^2 + 5} \right] \in C^2(\reals^N)
\end{flalign}
